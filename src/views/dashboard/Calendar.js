...
class Calendar1 extends React.Component {
  ...
  getCalendarData = async () => {
    const response = await fetch(
      `https://${process.env.host}:${process.env.port}/calendar/get_events`,
      {
        method: "GET",
        credentials: "include",
      }
  ...
  refreshData (action, eventItem) {
    switch (action) {
      case "update":
        fetch(
          `https://${process.env.host}:${process.env.port}/calendar/update_event`,
          {
            method: "POST",
            mode: "cors",
            headers: {
      ...
      case "insert":
        fetch(
          `https://${process.env.host}:${process.env.port}/calendar/insert_event`,
          {
            method: "POST",
            mode: "cors",
            headers: {
